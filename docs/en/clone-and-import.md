...
`git clone https://github.com/ByteLegendQuest/java-interface-mixin`

A project directory named `java-interface-mixin` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-interface-mixin` directory which is created just now.
